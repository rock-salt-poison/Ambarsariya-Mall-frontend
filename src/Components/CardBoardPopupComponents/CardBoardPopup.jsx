...
function CardBoardPopup({ open, handleClose,texturedSheet, iconSrc, title, body_content, optionalCName }) {
  const handleContentClick = (e) => {
    e.stopPropagation(); // Stop click propagation for content
  };

  return (
    <Dialog open={open} onClose={handleClose} className={`${texturedSheet ? 'textured_sheet_popup' : 'card_board_popup'} ${optionalCName ? optionalCName:''}`}>
